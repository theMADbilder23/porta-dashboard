...
function getWalletPrimaryId(wallet) {
...
function serializeWalletForBranch(wallet, snapshot, branchKey, totalPortfolioValue) {
  ...
  return {
    wallet_id: getWalletPrimaryId(wallet),
...
async function fetchWalletsWithFallback() {
  const lowercaseResult = await supabase.from("wallets").select("*");

  if (!lowercaseResult.error) {
    return lowercaseResult;
  }

  const lowerMessage = safeString(lowercaseResult.error?.message).toLowerCase();

  if (
    lowerMessage.includes('relation "wallets" does not exist') ||
    lowerMessage.includes("could not find the table")
  ) {
    const capitalizedResult = await supabase.from("Wallets").select("*");
...
export default async function handler(req, res) {
  ...
  try {
    ...
    for (const wallet of wallets) {
      const walletId = getWalletPrimaryId(wallet);
      if (!walletId) continue;
